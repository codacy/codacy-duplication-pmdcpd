...
import codacy.docker.api.{DuplicationConfiguration, DuplicationConfigurationValue, Source}
...
object Cpd extends DuplicationTool {
  override def apply(path: Source.Directory,
    ...
    Try {
      ...
      val directoryPath = Paths.get(path.path)
      val configurationOpt = getConfiguration(language, options)
      val cpd = configurationOpt.fold(new CPD(new CPDConfiguration())) { configuration =>
        new CPD(configuration)
      }
      ...
      cpd.getMatches.asScala.map(matchToClone(_, Paths.get(path.path)))
    } match {
  ...
  private def getConfiguration(languageOpt: Option[Language], options: Map[DuplicationConfiguration.Key, DuplicationConfiguration.Value]) = {

    val ignoreAnnotations = true
    val skipLexicalErrors = true

    val configOpt: Option[(net.sourceforge.pmd.cpd.Language,Int)] = languageOpt.flatMap {
      ...
      case Languages.Scala =>
        ...
        object ScalaL extends AbstractLanguage(cpdScala.getName, cpdScala.getTerseName, ScalaTokenizer, cpdScala.getExtensions.asScala: _*)
        Some((ScalaL, 50))
  ...
  private def matchToClone(m: Match, rootDirectory: Path): DuplicationClone = {
    val files: mutable.Set[DuplicationCloneFile] = m.getMarkSet.asScala.map { mark =>
    ...
    DuplicationClone(m.getSourceCodeSlice, m.getTokenCount, m.getLineCount, files.toSeq)
  }
