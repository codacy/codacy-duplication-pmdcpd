...
object Cpd extends DuplicationTool {
  ...
  private def runWithConfiguration(config: pmd.cpd.CPDConfiguration, directory: Path)(
    errStream: PrintStream): List[DuplicationClone] = {
  ...
  private def getLanguages(language: Option[Language]): Try[List[(pmd.cpd.Language, Int)]] = {
    ...
    val languagesOpt: Option[Try[List[(cpd.Language, Int)]]] =
      language.map { lang =>
        convertLanguageToConfig(lang) match {
    ...
    languagesOpt.getOrElse(Success(allLanguages.flatMap(convertLanguageToConfig)))
  }

  private def convertLanguageToConfig(language: Language): Option[(pmd.cpd.Language, Int)] = {
    language match {
      ...
      case Languages.Scala =>
        ...
        object ScalaL
            extends pmd.cpd.AbstractLanguage(
              cpdScala.getName,
  ...
  private def matchToClone(m: pmd.cpd.Match, rootDirectory: Path): DuplicationClone = {
