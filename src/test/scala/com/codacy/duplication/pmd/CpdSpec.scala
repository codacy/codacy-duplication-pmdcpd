...
class CpdSpec extends Specification {
  ...
  "Cpd" should {
    "get duplication" in {
      " without defining a language (directory has scala files)" in noLanguageTest()
  ...
  private def testClone(clone: DuplicationClone)(
    ...
    forall(clone.files) { file =>
      (file.filePath must not startWith "/") and
        (file.filePath must not contain dir)
    }
  }
  ...
  def cSharpTest(): MatchResult[Try[List[DuplicationClone]]] = {
    ...
    clonesTry must beLike {
      case Success(clones) =>
        ...
        val clone = clones.head
        testClone(clone)(codePath, 24, 66, 2)

        val secondClone = clones(1)
        testClone(secondClone)(codePath, 9, 53, 2)
    }
  }
